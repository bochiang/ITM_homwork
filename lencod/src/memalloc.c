...
int get_mem2D( pel_t ***array2D, int rows, int columns )
{
    ...
    if( ( *array2D      = ( pel_t** )calloc( rows,        sizeof( pel_t* ) ) ) == NULL )
    {
        no_mem_exit( "get_mem2D: array2D" );
    }

    if( ( ( *array2D )[0] = ( pel_t* )calloc( columns*rows,sizeof( pel_t ) ) ) == NULL )
    {
        no_mem_exit( "get_mem2D: array2D" );
    }
    ...
    return rows*columns*sizeof( pel_t );
}
...
int get_mem3D( pel_t ****array3D, int frames, int rows, int columns )
{
    int  j;

    if( ( ( *array3D ) = ( pel_t*** )calloc( frames,sizeof( pel_t** ) ) ) == NULL )
    {
    ...
    return frames*rows*columns*sizeof( pel_t );
}
...
void free_mem3D( pel_t ***array3D, int frames )
{
    int i;
